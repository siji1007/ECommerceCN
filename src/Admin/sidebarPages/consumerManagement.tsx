...
const ConsumerManagement: React.FC = () => {
    ...
    const confirmDeleteAccount = async () => {
        if (userToDelete !== null) {
            try {
                const response = await fetch(`${serverURL}/api/users/${userToDelete}`, {
                    method: "DELETE",
                });
                if (response.ok) {
    ...
    const handleViewAddress = (latitude: number, longitude: number) => {
        alert(`Latitude: ${latitude}\nLongitude: ${longitude}`);
    };

    const handleViewAccount = (user: any) => {
        setSelectedUser(user);
        setIsModalOpen(true); // Open the modal for viewing user details
    };

    const handleCloseModal = () => {
        setIsModalOpen(false); // Close the user details modal
    };
    ...
    const handleCloseMapModal = () => {
        setIsMapModalOpen(false); // Close the map modal
    };
    ...
    return (
        <div className="p-6">
            ...
            {isMapModalOpen && (
            <p className="absolute bottom-5 left-1/2 transform -translate-x-1/2 z-50 text-white text-sm font-semibold bg-black bg-opacity-60 p-2 rounded-md">
    ...
    <div className="fixed inset-0 bg-gray-800 bg-opacity-50 flex justify-center items-center w-full h-full">
        ...
        <div className="bg-white p-0 rounded-none w-full h-full">
           

            {/* Map container */}
            <MapContainer 
                center={[13.41, 122.56]} // Center map on the Philippines
                zoom={6} // Set zoom level to display the whole Philippines
                style={{ height: "100%", width: "100%" }} // Make the map fill the entire screen
            >
                <TileLayer
                    url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
                    attribution='&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
                />
                {/* Map all addresses and add a marker for each */}
                {addresses.map((address, index) => (
                    <Marker
                        key={index}
                        position={[address.latitude, address.longitude]} // Set the position of each marker
                        icon={new L.Icon({ iconUrl: Pin, iconSize: [30, 30], iconAnchor: [15, 0] })}
                    >
                        <Popup>
