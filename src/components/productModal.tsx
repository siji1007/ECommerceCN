...
const ProductModal: React.FC<ProductModalProps> = ({ isOpen, product, onClose, onAddToCart, onBuyNow }) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.session_cookie && data.user_id) {
          ...
          const checkCartResponse = await fetch(host + '/api/cart/check', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
          ...
          // If the product check fails, add the product to the cart directly
          if (!checkCartResponse.ok) {
            // Add product to the cart directly without checking existence
            await addProductToCart(userId, product, quantity);
          } else {
            const checkCartData = await checkCartResponse.json();
            if (checkCartData.exists) {
              // Product exists in the cart, update quantity
              const updatedQuantity = checkCartData.existing_quantity + quantity;
              const updatedTotalPrice = (product.prod_price - product.prod_disc_price) * updatedQuantity;
  
              // Update cart with new quantity
              const updateCartResponse = await fetch(host + '/api/cart/update', {
                method: 'POST',
                headers: {
                  'Content-Type': 'application/json',
                },
              ...
              if (updateCartResponse.ok) {
                alert('Product quantity updated in the cart! Navigating to your cart...');
  ...
  const addProductToCart = async (userId: string, product: any, quantity: number) => {
    try {
      const addToCartResponse = await fetch(host + '/api/cart/add', {
        ...
        body: JSON.stringify({
          cart_id: null, // Assuming cart_id is auto-generated on the server side
          us_id: userId,
          product_id: product.prod_id,
...
const handleCartNow = async (product, quantity) => {
  try {
    const response = await fetch(host + '/api/get-current-session', {
      method: 'GET',
      headers: {
      ...
      if (data.session_cookie && data.user_id) {
        ...
        try {
          ...
          if (addToCartResponse.ok) {
            ...
            const errorData = await addToCartResponse.json();
            if (errorData.message === 'You already have this product in your cart!') {
